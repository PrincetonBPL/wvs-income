...
* This doesn't work (false axis labels etc)
*local thisindepvar "growth" 
*local thisindepvar_graph "growth" 

local thisindepvar "lgdp" 
local thisindepvar_graph "gdp" 
...
foreach v of varlist shapefateyrself_hat wouldntwork_hat intrinsic_hat trustmeetfirsttime_hat helpfamily_hat lonely_hat lifemeaningless_hat livedaytoday_hat risktaking_hat hap_hat sat_hat {
	...
	correl `v' `thisindepvar' 
	local p= string(min(2*ttail(r(N)-2, abs(r(rho))*sqrt(r(N)-2)/ sqrt(1-r(rho)^2)),1) , "%5.4f")
	local rho=string(r(rho),"%3.2f")
	
	reg `v' `thisindepvar', r
	local n=`e(N)'
	predict `v'_fit
	local b=string(_b[`thisindepvar'],"%3.2f")
	local a=string(_b[_cons],"%3.2f")
	local se=string(_se[`thisindepvar'],"%3.2f")
	lowess `v' `thisindepvar', nograph generate(`v'_low)

	qui sum `thisindepvar' if `v'~=.
...
dis "done separating"
	...
	* AF = 1, AS = 2, EU = 3, NA = 4, OC = 5, SA = 6
		...
		(scatter `v'? `thisindepvar_graph', mcolor(black yellow blue red green cranberry) mlcolor(black ..) msize(2 ..) legend(on) )
		(line `v'_fit `thisindepvar_graph', sort lpattern(solid) lcolor(black))
		if `v' ~= .
		,
		xscale(log)
	...
	#delimit cr		
	drop `v'_fit `v'_low
	graph export "figs/macro1_`thisindepvar'_`v'.eps", replace
	graph save "figs/macro1_`thisindepvar'_`v'", replace 
...
graph display macro1, xsize(6) ysize(8) 
   
   
* graph save "figs/macro1.gph", replace
graph export "figs/macro1_`thisindepvar'_2013.eps", replace
