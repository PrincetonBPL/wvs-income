...
*/
	* Make overall trend line
	...
	dis "again: `rho'"
    	#delimit ;
		twoway 				
					(line `v'_mean inc if continentnum==1, lcolor(black) lpattern(solid)) 
					(line `v'_mean inc if continentnum==2, lcolor(yellow) lpattern(solid)) 
					(line `v'_mean inc if continentnum==3, lcolor(blue) lpattern(solid)) 
					(line `v'_mean inc if continentnum==4, lcolor(red) lpattern(solid)) 
					(line `v'_mean inc if continentnum==5, lcolor(green) lpattern(solid)) 
					(line `v'_mean inc if continentnum==6, lcolor(cranberry) lpattern(solid)) 
		...
		,
		xtitle("Income decile within country")
		...
		note("N = `n'" "r = `rho', p = `p'", ring(0) pos(7))
				note("A", ring(1) pos(10) size(*2))

	;
	#delimit cr		
	drop `v'_fit* 
	graph export "figs/micro_2013_`v'_Science_Revision1B.eps", replace
	graph save "figs/micro1_`v'_Science_Revision1B", replace 
}
